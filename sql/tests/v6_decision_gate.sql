...
begin
  ...
  where model_family='COUNT_NULL_MODEL'
    ...
    and metrics ? 'decision_gate'
  order by created_at desc,id desc limit 1;

  if v_exp is null or v_cutoff is null then
    raise exception 'V6 DECISION FAIL: no decision gate summary';
  end if;
  ...
  where model_version='DECISION_PREREQUISITE_GATE_V1'
    and as_of=v_cutoff
    and (p_structural is not null
      or p_bottleneck is not null
      or p_economic_capture is not null
      or p_fundamentals_exceed_expectations is not null
      or expected_residual_return is not null);
  ...
  if v_rows=0 then raise exception 'V6 DECISION FAIL: no NO_ACTION rows'; end if;
  if v_bad_decision<>0 then raise exception 'V6 DECISION FAIL: % rows are actionable',v_bad_decision; end if;
