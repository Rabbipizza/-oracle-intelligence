...
This worker consumes null-model p-values already written to
oracle_v6_signal_null_models, applies the existing Benjamini-Hochberg selector
from oracle_v6_scientific_core, and persists the entire hypothesis family to
oracle_v6_multiple_testing before any downstream trend can be inserted.
"""
...
def _bh_adjusted_p_values(p_values: Sequence[float]) -> list[float]:
    """Return BH adjusted p-values for audit logging only.

    Selection itself is always delegated to the canonical
    benjamini_hochberg() implementation in oracle_v6_scientific_core.
    """
    n = len(p_values)
    if n == 0:
        return []
...
def run_fdr(db_url: str, alpha: float = DEFAULT_ALPHA) -> FDRResult:
    ...
    # SQLAlchemy/psycopg expects postgresql+psycopg for explicit driver routing.
    engine_url = db_url
    ...
    with engine.begin() as conn:
        as_of = conn.execute(
            text("select max(as_of) from public.oracle_v6_signal_null_models where p_value is not null")
        ).scalar_one_or_none()
        if as_of is None:
            raise RuntimeError("No null-model p-values available")
        ...
        rows = conn.execute(
            text(
                ...
                where as_of = :as_of and p_value is not null
                order by signal_key
                """
            ),
            {"as_of": as_of},
        ...
        if not rows:
            raise RuntimeError("Latest null-model family is empty")
        ...
        if experiment_id is None:
            raise RuntimeError("No COUNT_NULL_MODEL experiment registered")
