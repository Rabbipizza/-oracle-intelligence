...
Links structural dependencies to PIT SEC evidence without scoring scarcity,
capture, return, or investment merit. It also distinguishes explicit supply-side
wording from demand-side exposure so a buyer of a scarce input is never presented
as a picks-and-shovels supplier merely because it mentions the constraint.
"""
...
RULES={
 ...
 "Advanced Semiconductor Packaging":({"PACKAGING","HBM"},r"\b(advanced packaging|wafer-level packaging|3d stacking)\b"),
...
def run(db_url):
 ...
 with eng.begin() as conn:
  ...
  evaluation_as_of=conn.execute(text(q),p).scalar_one_or_none()
  ...
  summary={"model_version":MODEL_VERSION,"matching_policy":"STRICT_EXPLICIT_DEPENDENCY_TERM_AND_ROLE_CUE","evaluation_as_of":evaluation_as_of.isoformat(),**{k:int(v or 0) for k,v in stats.items()},"probabilities_written":0,"state":"STRUCTURAL_EXPOSURE_CANDIDATES_AVAILABLE" if stats['evidence_links'] else 'NO_STRUCTURAL_COMPANY_LINKS'}
