...
(capacity/supply/constraint). Otherwise it remains a research candidate.
"""
...
MODEL_VERSION='ECONOMIC_CAPTURE_EVIDENCE_V1'
...
def run(db_url):
    ...
    with eng(db_url).begin() as conn:
        ...
        raw=conn.execute(text("select id,document_id,ticker,evidence_key,excerpt,source_url,coalesce(available_at,known_at,created_at) available_at from public.oracle_v4_sec_economic_evidence where coalesce(available_at,known_at,created_at)<=:a and evidence_key=any(:keys) order by id"),{'a':cutoff,'keys':sorted(PULL_KEYS|ABILITY_KEYS)}).mappings().all()
        conn.execute(text("delete from public.oracle_v6_economic_capture_evidence where experiment_id=:e and evaluation_as_of=:a and model_version=:m"),{'e':exp,'a':cutoff,'m':MODEL_VERSION})
        payload=[]
        for s in suppliers:
            ...
            pull=bool(keys & PULL_KEYS); ability=bool(keys & ABILITY_KEYS)
            backed=len(docs)>=2 and pull and ability
            meta={'semantics':'descriptive_capture_evidence_not_probability','independent_documents':len(docs),'pull_evidence':pull,'ability_evidence':ability,'probability_synthesized':False}
            payload.append({'experiment_id':exp,'evaluation_as_of':cutoff,'bottleneck_node':node,'ticker':ticker,'status':'CAPTURE_EVIDENCE_BACKED' if backed else 'CAPTURE_EVIDENCE_CANDIDATE','n':len(docs),'types':json.dumps(sorted(keys)),'ids':json.dumps([int(r['id']) for r in ev]),'pull':pull,'ability':ability,'model':MODEL_VERSION,'meta':json.dumps(meta,sort_keys=True)})
        if payload:
            conn.execute(text("insert into public.oracle_v6_economic_capture_evidence (experiment_id,evaluation_as_of,bottleneck_node,ticker,evidence_status,independent_evidence_count,evidence_types,evidence_ids,supplier_role_confirmed,demand_backlog_evidence,capacity_evidence,pricing_power_evidence,model_version,metadata) values (:experiment_id,:evaluation_as_of,:bottleneck_node,:ticker,:status,:n,cast(:types as jsonb),cast(:ids as jsonb),true,:pull,:ability,false,:model,cast(:meta as jsonb))"),payload)
        backed=sum(1 for p in payload if p['status']=='CAPTURE_EVIDENCE_BACKED')
        summary={'model_version':MODEL_VERSION,'evaluation_as_of':cutoff.isoformat(),'tension_supplier_candidates':len(payload),'capture_evidence_backed':backed,'capture_probabilities_written':0,'pricing_power_probabilities_written':0,'state':'CAPTURE_EVIDENCE_AVAILABLE_UNCALIBRATED' if payload else 'NO_TENSION_SUPPLIER_CANDIDATES'}
