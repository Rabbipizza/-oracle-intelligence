...
USER_AGENT = os.environ.get("ORACLE_SEC_USER_AGENT","ORACLE-V6 research https://github.com/Rabbipizza/-oracle-intelligence")
...
def _fetch_companyfacts(cik):
    req=urllib.request.Request(SEC_BASE.format(cik=str(cik).zfill(10)),headers={"User-Agent":USER_AGENT,"Accept-Encoding":"identity","Accept":"application/json"})
...
def _iter_rows(ticker,cik,payload):
    ...
    for taxonomy,concept in ALL_TAGS:
        ...
        for item in node.get("units",{}).get(unit,[]):
            filed,end,val=_parse_date(item.get("filed")),_parse_date(item.get("end")),item.get("val")
...
def _materialize(conn,tickers,cutoff):
    ...
    for (ticker,period_end),facts in by_period.items():
        ...
        debt=sum((Decimal(str(x["value"])) for x in debt_parts),Decimal("0")); comps=[cash,shares]+debt_parts; as_of=max(x["available_at"] for x in comps)
        snapshots.append({"ticker":ticker,"as_of":as_of,"period_end":period_end,"shares":Decimal(str(shares["value"])),"cash":Decimal(str(cash["value"])),"debt":debt,"source_components":json.dumps([{"taxonomy":x["taxonomy"],"concept":x["concept"],"accession":x["accession"],"available_at":x["available_at"].isoformat()} for x in comps],sort_keys=True)})
    conn.execute(text("delete from public.oracle_v6_position_snapshots where model_version=:model and ticker=any(:tickers)"),{"model":MODEL_VERSION,"tickers":tickers})
    if snapshots: conn.execute(text("insert into public.oracle_v6_position_snapshots (ticker,as_of,fiscal_period_end,shares_outstanding,cash,debt,source_components,source_quality,model_version,metadata) values (:ticker,:as_of,:period_end,:shares,:cash,:debt,cast(:source_components as jsonb),'SEC_COMPANYFACTS_PIT_CONSERVATIVE',:model,jsonb_build_object('availability_quality','ESTIMATED_CONSERVATIVE_FROM_FILED_DATE','shares_definition','dei.EntityCommonStockSharesOutstanding only','debt_definition','preferred current + noncurrent + short borrowing tags'))"),[dict(s,model=MODEL_VERSION) for s in snapshots])
...
def run(db_url):
    ...
    with engine.begin() as conn:
        ...
        cutoff=conn.execute(text("select evaluation_as_of from public.oracle_v6_evaluation_runs where experiment_id=:exp and (:run_id is null or external_run_id=:run_id) order by evaluation_as_of desc,id desc limit 1"),{"exp":int(exp),"run_id":run_id}).scalar_one_or_none()
        if cutoff is None: raise RuntimeError("no evaluation clock")
        targets=conn.execute(text("select distinct c.ticker,u.cik from public.oracle_v6_company_capture c join public.oracle_v5_universe u on u.ticker=c.ticker where c.model_version=:capture_model and c.as_of=:cutoff and u.cik is not null and btrim(u.cik)<>'' order by c.ticker"),{"capture_model":CAPTURE_MODEL,"cutoff":cutoff}).mappings().all()
    fetched=0; failed=[]; inserted=0
    for t in targets:
        ticker,cik=str(t["ticker"]),str(t["cik"])
        try:
            payload=_fetch_companyfacts(cik); facts=list(_iter_rows(ticker,cik,payload))
            with engine.begin() as conn:
                if facts:
    ...
    with engine.begin() as conn:
        ...
        cov=conn.execute(text("select count(*) snapshots,count(distinct ticker) tickers from public.oracle_v6_position_snapshots where model_version=:model and as_of<=:cutoff and ticker=any(:tickers)"),{"model":MODEL_VERSION,"cutoff":cutoff,"tickers":tickers}).mappings().one()
        ...
        conn.execute(text("update public.oracle_v6_experiments set metrics=jsonb_set(coalesce(metrics,'{}'::jsonb),'{sec_position_pit}',cast(:summary as jsonb),true) where id=:exp"),{"summary":json.dumps(summary,sort_keys=True),"exp":int(exp)})
